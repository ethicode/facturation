...
function InvoiceDetailPage() {
  ...
  useEffect(() => {
    ...
    async function fetchInvoices() {
      try {
        ...
        if (isMounted) {
          setApiError(error.message || 'Impossible de charger la facture.')
        }
      }
    }
  ...
  const handleTransition = async (nextStatus) => {
    try {
      ...
      setApiError(error.message || 'Impossible de mettre à jour le statut de la facture.')
    }
  }
  ...
  return (
    <Stack spacing={2.5}>
      <PageHeader
        title={`Résolution du process - ${invoice.id}`}
        subtitle="Cliquez sur une facture pour afficher son détail, son workflow et exécuter les actions."
      />
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
              {allowedTransitions.length > 0 ? (
                ...
                <Alert severity="info" sx={{ width: '100%' }}>
                  Aucune action autorisée pour le rôle {roleLabels[activeRole]} sur le statut {invoice.statut}.
                </Alert>
              )}
            </Stack>
            {getNextStatuses(invoice.statut).length === 0 && (
              <Alert severity="success">Cette facture est finalisée. Aucune action supplémentaire.</Alert>
            )}
          </Stack>
        </CardContent>
      </Card>
